...
export async function getEpisodes(season) {
  ...
  return getEpisodes.p
    .then((eps) => !seasonNum ? eps : eps?.filter(ep => ep.season?.number === seasonNum));
}
...
export async function getLatestEpisode({filter = published, season} = {}) {
  const eps = (await getEpisodes(season)).filter(filter);
...
export async function getSeasons() {
  async function fetchSeasons() {
    ...
    const { data: { collection: seasons }} = response;
